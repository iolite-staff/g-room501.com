...
echo ini_get('mbstring.encoding_translation');
...
print_r($post);
	if(isset($post['dat0yy'])){
		...
		if($no == ""){
			...
			$no = $dat[0];
			//旧Noのファイルを削除
			unlink(DATPATH."diary/".$old_no.".txt");
		}
		//--- Guitar room 501 Diaryファイル更新
		if($post['text'] != ""){
			//改行コードごと配列に格納
// change maruyama 160705 from
			$txt = preg_split("/[\r\n]/", trim($post['text']));
//			$txt = split("\n", trim($post['text']));
// change maruyama 160705 to
			$cnt = count($txt);
			//95行以上は切り捨て
			if($cnt > 95){
				$cnt = 95;
...
<title>Guitar room 501 Diary</title>
...
<form name="no_sel" method="get" action="diary.php">
  <div align="center">
    <select name="no">
<?php
...
<?php
	...
	for($fp=0;$fp<$max_fp;$fp++){
		...
		if($index[$fp] != ""){
			$datpath = DATPATH."diary/".$index[$fp].".txt";
			//Datファイル存在チェック
...
<?=$msg?>
  <div align="center">
    <form name="Update" method="post" action="diary.php?no=<?=$no?>">
...
<form name="Delete" method="post" action="diary.php">
